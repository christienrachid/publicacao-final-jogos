...
export default class Slime extends me.Entity {
  constructor(x, y, jogador) {
    ...
    if (!Slime.ultimoSomHit) Slime.ultimoSomHit = 0;
    if (!Slime.ultimoSomMorte) Slime.ultimoSomMorte = 0;
  ...
  criarAnimacoes() {
    this.renderable.addAnimation("andar_direita", [0, 1], 100);
    this.renderable.addAnimation("andar_esquerda", [4, 5], 100);

    this.renderable.addAnimation("atacar_direita", [8, 9, 10], 100);
    this.renderable.addAnimation("atacar_esquerda", [12, 13, 14], 100);
  ...
  tomarDano(quantidade) {
    ...
    if (this.vida <= 0) {
      ...
      if (agora - Slime.ultimoSomHit > 100) {
        me.audio.play("8bitEnemyHit", false, null, 0.3);
        Slime.ultimoSomHit = agora;
      }
      this.piscar();
    }
  }
  ...
  morrer() {
    ...
    const agora = Date.now();
    if (agora - Slime.ultimoSomMorte > 100) {
      me.audio.play("8bitEnemyDeath", false, null, 0.4);
      Slime.ultimoSomMorte = agora;
    }
