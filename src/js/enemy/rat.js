...
export default class Rat extends me.Entity {
  constructor(x, y, jogador) {
    ...
    if (!Rat.ultimoSomHit) Rat.ultimoSomHit = 0;
    if (!Rat.ultimoSomMorte) Rat.ultimoSomMorte = 0;
  ...
  tomarDano(quantidade) {
    ...
    if (this.vida <= 0) {
      ...
      if (agora - Rat.ultimoSomHit > 100) {
        me.audio.play("8bitEnemyHit", false, null, 0.3);
        Rat.ultimoSomHit = agora;
      }
      this.piscar();
    }
  }
  ...
  morrer() {
    ...
    const agora = Date.now();
    if (agora - Rat.ultimoSomMorte > 100) {
      me.audio.play("8bitEnemyDeath", false, null, 0.4);
      Rat.ultimoSomMorte = agora;
    }
